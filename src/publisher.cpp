...
void Publisher::advertise()
{
  ...
  try {
    ...
    QML_ROS2_PLUGIN_ERROR( "Failed to create publisher for topic '%s': %s",
                           topic_.toStdString().c_str(), ex.what() );
  } catch ( std::exception &ex ) {
    QML_ROS2_PLUGIN_ERROR( "Failed to create publisher for topic '%s': %s",
                           topic_.toStdString().c_str(), ex.what() );
  } catch ( ... ) {
    QML_ROS2_PLUGIN_ERROR( "Failed to create publisher for topic '%s': Unknown error",
